/*
  프로그램 개요:
  이 프로그램은 C 언어로 구현한 간단한 단어 자동완성 시뮬레이터입니다.

  주요 기능:
  - 사용자가 문자 하나씩 입력하면 현재 입력된 문자열을 스택으로 관리합니다.
  - Undo(/u) 명령어로 최근 입력한 문자를 되돌릴 수 있고,
    Redo(/r) 명령어로 Undo한 문자를 다시 복원할 수 있습니다.
  - 현재 입력된 문자열을 기준으로 미리 정의된 사전(dictionary)에서
    해당 문자열로 시작하는 단어들을 자동으로 추천하여 출력합니다.
  - '/q' 명령어로 프로그램을 종료할 수 있습니다.

  내부 구조:
  - 입력 문자들을 저장하는 입력 스택(inputStack)과 Undo한 문자를 저장하는 Undo 스택(undoStack)을 사용합니다.
  - 사용자가 새 문자를 입력할 때마다 Undo 스택은 초기화되어
    Redo 기록이 초기화됩니다.
  - 추천 단어는 현재 입력된 문자열이 단어의 접두사인지 검사하여 출력합니다.

  사용자 인터페이스:
  - 문자 입력 또는 '/'로 시작하는 명령어 입력 방식
  - 명령어: /u (Undo), /r (Redo), /q (종료)
  - 실시간으로 추천 단어 목록을 보여줌
*/
